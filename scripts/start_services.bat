...
REM Set Django settings module to use simple development settings
set DJANGO_SETTINGS_MODULE=order_fulfillment.settings_dev
...
start "Django Server" cmd /k "cd /d "%CD%" && set DJANGO_SETTINGS_MODULE=order_fulfillment.settings_dev && venv\Scripts\activate.bat && python manage.py runserver"
...
start "Celery Worker" cmd /k "cd /d "%CD%" && set DJANGO_SETTINGS_MODULE=order_fulfillment.settings_dev && venv\Scripts\activate.bat && celery -A order_fulfillment worker -l info"
...
start "Celery Beat" cmd /k "cd /d "%CD%" && set DJANGO_SETTINGS_MODULE=order_fulfillment.settings_dev && venv\Scripts\activate.bat && celery -A order_fulfillment beat -l info"
